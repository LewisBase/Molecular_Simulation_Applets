...
const double r = 3.455e-15;				 // 空气阻尼系数
...
int main()
{
	ofstream pout("antioutput.dat", ios::out | ios::app);
    pout << "v0 = " << v0 << endl;
	pout.close();
	// 批量文件读入与输出
	...
	for (int file = 0; file < 9; file++)  // 同上
	{
		...
		sprintf(input,"antienergy%d.dat",file);
		...
		vi[0] = v0;													// 初始速度
		double vo = v0-1;										// 迭代预估值
		double xt = 4.0;											// 最终位置
		double vt = 99.99;											// 最终速度
		double vt0 = 0.0;
		for (i = 1; i < Num; i++)
		{
			...
			if (i<(Num-1))
			{
				...
				if (vi[i] <= 0)
				{
					...
					vt = vi[i];
					vt0 = vi[i-1];
					cout << "velocity start to reduce less than zero" << endl;
					cout << "The coordinate where velocity start to reduce less than zero: " << xi[i] << endl;
					cout << "The next velocity: " << vi[i] << endl << endl << endl;
					break;
				}
			}
            vo = (vo + vi[i])/2;
		...
		ofstream fout("antioutput.dat", ios::out | ios::app);
        fout << "molname" << "\t" << "xt" << "\t" << "vt-1" << "\t" << "vt" << endl;
		fout << molname << "\t" << xt << "\t" << vt0 << "\t" << vt << endl;
		fout.close();
